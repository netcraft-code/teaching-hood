...
const JobCard = () => {
  ...
  const setCity       = (v) => { cityRef.current       = v;    setSelectedCity(v);       };
  const setJobType    = (v) => { jobTypeRef.current     = v;    setSelectedJobType(v);    };
  const setExperience = (v) => { experienceRef.current  = v;    setSelectedExperience(v); };
  const setPosted     = (v) => { postedRef.current      = v;    setPostedDate(v);         };
  const setSearch     = (v) => { searchRef.current      = v;    setSearchQuery(v);        };
  const setBoard      = (v) => { boardRef.current       = v;    setSelectedBoard(v);      };
  const setSalary     = (v) => { salaryRef.current      = v;    setSelectedSalary(v);     };
  const setSubject    = (v) => { subjectRef.current     = v;    setSelectedSubject(v);    };
  const setGrade      = (v) => { gradeRef.current       = v;    setSelectedGrade(v);      };
  const setPage       = (v) => { pageRef.current        = v;    setCurrentPage(v);        };
  ...
  const buildFilters = ({ page } = {}) => {
    const range = salaryRef.current !== null ? SALARY_RANGES[salaryRef.current] : null;
    return {
      page:       page ?? pageRef.current,
      city_id:    cityRef.current?.value || "all",
      job_type:   jobTypeRef.current    !== "all" ? jobTypeRef.current    : "all",
      experience: experienceRef.current !== "all" ? experienceRef.current : "all",
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [subRes, gradeRes] = await Promise.all([
        fetch("https://teaching-hood-backend.netcraftglobal.com/api/subjects").then(r => r.json()),
        fetch("https://teaching-hood-backend.netcraftglobal.com/api/gradelevels").then(r => r.json()),
      ]);
      ...
      if (subjectParam) {
        const exists = subList.some(s => String(s.id) === String(subjectParam));
        if (exists) setSubject(String(subjectParam));
      }
      if (gradeParam) {
        const exists = gradeList.some(g => String(g.id) === String(gradeParam));
      ...
      if (cityParam) {
        try {
          ...
          const list = res?.data?.data?.data?.map(c => ({ value: c.id, label: c.name })) || [];
          if (list[0]) {
  ...
  useEffect(() => {
    ...
    selectedCity, selectedJobType, selectedExperience,
    postedDate, searchQuery, selectedBoard,
    selectedSubject, selectedGrade, selectedSalary,
  ]);
  ...
  const loadCities = async (inputValue) => {
    ...
    try {
      ...
      return res?.data?.data?.data?.map(c => ({ value: c.id, label: c.name })) || [];
    } catch {
      return [];
    }
  };
  ...
  const formatSalary = (min, max) => {
    ...
    const found = SALARY_RANGES.find(r => r.min === Number(min) && r.max === Number(max));
    return found ? `Salary: ${found.label}/month` : `Salary: ₹${min} - ₹${max}/month`;
  };

  const getJobTitle = (job) => {
    if (job.subject_name && job.grade_name) return `${job.grade_name} ${job.subject_name} Teacher`;
  ...
  const likeUnlikedJob = async (jobId) => {
    try {
      ...
      if (response.data.status)
        setJobs(prev => prev.map(j => j.id === jobId ? { ...j, is_liked: !j.is_liked } : j));
  ...
  const applyJob = async (jobId) => {
    ...
    try {
      ...
      if (response.data.status) {
        ...
        setShowPopup(true);
        setJobs(prev => prev.map(j => j.id === jobId ? { ...j, is_applied: !j.is_applied } : j));
      }
    } catch {
  ...
  const FilterSidebar = ({ isMobile = false }) => (
    <div className={`bg-white rounded-[24px] shadow-sm p-6 ${isMobile ? "" : "sticky top-6"}`}>
      <div className="flex items-center justify-between mb-6">
        ...
        {isMobile && (
          <button onClick={() => setIsFilterOpen(false)} className="lg:hidden p-2 hover:bg-gray-100 rounded-lg">
            <X className="w-6 h-6" />
          </button>
        )}
      </div>
      ...
      <div className="mb-6">
        ...
        <AsyncSelect
          ...
          noOptionsMessage={({ inputValue }) =>
            inputValue.length < 2 ? "Type at least 2 characters" : "No cities found"
          }
        />
      </div>
      ...
      <div className="mb-6">
        ...
        <select value={selectedGrade} onChange={e => setGrade(e.target.value)}
          className="w-full px-4 py-3 border rounded-xl bg-gray-50">
          ...
          {[
            ...
            return (
              <label key={item.value}
                className={`flex items-center justify-between p-4 rounded-lg border cursor-pointer transition ${isActive ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-300"}`}>
                <div className="flex items-center gap-3">
                  <span className={`w-5 h-5 flex items-center justify-center rounded-full border ${isActive ? "border-blue-600" : "border-gray-300"}`}>
                    {isActive && <span className="w-2.5 h-2.5 bg-blue-600 rounded-full" />}
                  </span>
                  <span className="text-gray-800 font-medium">{item.label}</span>
                </div>
                <input type="radio" name="jobType" value={item.value} checked={isActive}
                  onChange={e => setJobType(e.target.value)} className="hidden" />
              </label>
            );
          })}
        </div>
          ...
          {[
            ...
            return (
              <label key={item.value}
                ...
                <input type="radio" name="experience" value={item.value} checked={isActive}
                  onChange={e => setExperience(e.target.value)} className="hidden" />
              </label>
            );
          })}
        </div>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          {SALARY_RANGES.map((item, index) => {
            ...
            return (
              <div key={index} onClick={() => setSalary(isActive ? null : index)}
                className={`p-3 border rounded-lg cursor-pointer ${isActive ? "bg-blue-50 border-blue-500" : ""}`}>
                {item.label}
              </div>
            );
          })}
          ...
          {[
            ...
            return (
              <label key={item.value}
                className={`flex items-center justify-between p-4 rounded-lg border cursor-pointer transition ${isActive ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-300"}`}>
                <span className="text-gray-800 font-medium">{item.label}</span>
                <input type="radio" name="postedDate" value={item.value} checked={isActive}
                  onChange={e => setPosted(e.target.value)} className="hidden" />
              </label>
            );
          })}
        </div>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          {BOARDS.map(b => {
            ...
            return (
              <div key={b} onClick={() => setBoard(isActive ? "all" : b)}
                className={`p-3 border rounded-lg cursor-pointer ${isActive ? "bg-blue-50 border-blue-500" : ""}`}>
                {b}
              </div>
            );
      ...
      <button onClick={() => { clearAllFilters(); if (isMobile) setIsFilterOpen(false); }}
  ...
  return (
    <div>
      <section className="py-12 md:py-16 lg:py-24 min-h-[40vh] bg-cover bg-center relative overflow-hidden"
          ...
          <div className="max-w-4xl mx-auto text-center">
            <div className="inline-flex items-center space-x-2 bg-white backdrop-blur-sm px-3 md:px-4 py-2 rounded-full mb-4 md:mb-6">
              ...
              <span className="text-xs md:text-sm text-gray-600">
                <span className="text-blue-500">{newJobs} new job{newJobs > 1 ? "s" : ""}</span> posted this week
              </span>
            </div>
            ...
            <p className="text-3xl md:text-4xl lg:text-6xl text-white mb-6 md:mb-10 max-w-2xl mx-auto"
              style={{ textShadow: "0px 4px 4px #00000040" }}>
              ...
              <div className="flex flex-col md:flex-row items-center bg-white rounded-2xl shadow-lg p-3 gap-3">
                <div className="flex items-center flex-1 w-full gap-3 px-2 md:px-4">
                  ...
                  <input type="text" value={searchQuery} onChange={e => setSearch(e.target.value)}
                    placeholder="Search by Subject, Grade, Location...." className="w-full outline-none text-sm md:text-base" />
                </div>
                <button onClick={() => { setPage(1); fetchJobs({ page: 1 }); }}
              ...
              <div className="space-y-4">
                {loading ? (
                  ...
                  <div className="text-center py-12 bg-white rounded-lg shadow-sm">
                    <p className="text-gray-600 text-base md:text-lg">No jobs found matching your filters</p>
                    <button onClick={clearAllFilters} className="mt-4 text-blue-600 hover:underline">Clear all filters</button>
                  </div>
                ) : (
                  jobs.map(job => (
                    <div key={job.id} className="bg-white rounded-2xl border-t-8 border-blue-400 shadow-lg p-4 md:p-6">
                        ...
                        <Link to={`/job/${job.id}`}>
                          <div className="flex items-start gap-3 md:gap-4">
                            <div className="w-12 h-12 md:w-16 md:h-16 bg-blue-100 rounded-2xl flex items-center justify-center flex-shrink-0">
                              <img src={findJobIcons.schoolIcon} className="w-6 h-6 md:w-8 md:h-8" />
                            </div>
                            <div>
                              <h3 className="text-lg md:text-xl font-semibold">{getJobTitle(job)}</h3>
                              <p className="text-sm md:text-base text-gray-600 mt-1">{job.school_name}</p>
                              <div className="flex items-center gap-1 mt-1 text-xs md:text-sm text-gray-600">
                                ...
                                <span>{job.total_applicants} applicant{job.total_applicants > 1 ? "s" : ""}</span>
                              </div>
                            </div>
                          </div>
                        </Link>
                        <button onClick={() => likeUnlikedJob(job.id)} className="transition-colors self-start">
                          <img src={findJobIcons.like} className={`w-6 h-6 ${job.is_liked ? "filter-red" : "opacity-40"}`} />
                        </button>
                      </div>
                      ...
                      <div className="mt-4">
                        <div className="flex flex-wrap gap-2 mb-4">
                          {job.board && (
                            <span className="px-3 py-1 bg-yellow-100 text-yellow-700 text-xs md:text-sm rounded-full">{job.board}</span>
                          )}
                        </div>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-4 text-xs md:text-sm text-gray-600">
                          <div className="flex items-center gap-1">
                            <img src={findJobIcons.findJobLocation} className="w-4 h-4 flex-shrink-0" />
                            <span className="truncate">{job.city_name}, {job.city?.state?.name}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <img src={findJobIcons.availableJobType} className="w-4 h-4 flex-shrink-0" />
                            <span className="truncate">{job.job_type}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <img src={findJobIcons.timeDuration} className="w-4 h-4 flex-shrink-0" />
                            <span className="truncate">{getTimeAgo(job.created_at)}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <img src={findJobIcons.totalExperience} className="w-4 h-4 flex-shrink-0" />
                            <span className="truncate">{job.experience_required} years</span>
                          </div>
                        </div>
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                          <div className="font-semibold text-gray-900" style={{ fontSize: "19px" }}>
                            {formatSalary(job.min_salary, job.max_salary)}
                          </div>
                          {job.is_applied ? (
                            <span className="bg-red-100 text-red-500 px-4 py-2 rounded-full text-sm">Applied</span>
                          ) : (
                            <div>
